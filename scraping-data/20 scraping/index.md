...
In order to answer the questions we will have to make some assumptions. Most importantly we are going to assume that the top 5 movies of each year are indicative of how good for movies that year is. So we will reformulate the question:
...
- You will use *BeautifulSoup* to parse the Document Object Model (DOM) if IMDB. We provide some scaffolding for the programming exercise. [BeautifulSoup documentation](https://www.crummy.com/software/BeautifulSoup/bs4/doc/)
...
1. For this assignment you will need to have the following Python 3 libraries installed: Requests, Pandas, and BeautifulSoup4.

2. To get you started we have provided you with some code (`scraper.py` and `helpers.py`). `helpers.py` contains the function `simple_get` for loading a webpage. The file `scraper.py` is the file you will be editing. To get started, it already contains some code that imports `simple_get` and uses it to load the correct IMDB address and prints the first line.
...
This is the introductory exercise to BeautifulSoup. We will try to guide you along as much as possible, but you should read up on documentation and get used to doing that. It is a useful skill and a big part of programming is self-­learning! Watch the following videos on HTML and the DOM. Ignore any reference to JavaScript (you may replace it in your mind with BeautifulSoup), as it will not be needed in our exercises.
...
Open up and read the `scraper.py`-file. Note that this is just some scaffolding, so you actually do not have to use this at all. As long as your code runs at the end of the day and produces the right results in a CSV file, we are happy.
...
- The function `extract_movies()` reads the header from the webpage and prints it. This print statement is just an example to get you started, but should be removed. The function should not print anything when it is finished.
...
The value for `start` increases by 50 every time. So first it's `start=1`, then `start=51`, then `start=101`, etc. We can use this to collect as many top rated movies as we would like.
